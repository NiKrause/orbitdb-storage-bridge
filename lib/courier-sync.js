 ...
 *
 * It is also expensive out of all proportion to a todo. Measured on a keyvalue
 * entry holding `{text, done, ts}`: the signed block is 519 B, and what the
 * delta plane actually puts on the air for one change is 1531 B once the
 * identity block travels with it. The operation inside that entry —
 * `{op: "PUT", key, value}`, which OrbitDB hands to every `update` listener
 * already — is **113 B** on the wire, tag and sender id and change id
 * included. Thirteen times smaller, and on a carrier moving half a kilobyte a
 * minute that is three minutes against fourteen seconds.
 *
