 ...
 *
 * ```
 * dial ok in 729 ms          /dns4/bitswap-v3.pinata.cloud/tcp/443/wss/p2p/Qmdv6y…
 * BITSWAP OK: 32 bytes in 263 ms
 * ```
 *
 * No credential in any of it, while that provider's HTTP gateway wants the
 * account's key. Lighthouse answered the same way over `webrtc-direct`
 * (0.47 s dial, 0.39 s bitswap) from a node with only browser transports.
 *
